...
export default function htmlCSSPlugin(userOptions = {}) {
      ...
      (fullMatch, p1, path) => {
        ...
        if (
          ...
          const id = rollup.emitChunk(join(parentDir, targetPath), {
            // Remove extension from name. We'll add it again in generateBundle.
            name: targetPath.replace(/\.[^\.]+$/, ''),
          });

          // This will be replaced with the real URL in generateBundle.
          return `#import.meta.ROLLUP_CHUNK_URL_${id}#`;
        }
        ...
        // This will be replaced with the real URL in generateBundle.
        return `#import.meta.ROLLUP_ASSET_URL_${assetCache.get(targetPath)}#`;
      },
    );
  ...
  return {
    ...
    load(id) {
      ...
      if (!(id.endsWith('.html') || id.endsWith('.css'))) return;
      // Cache name, so we know what to handle in generateBundle
      nonJSChunks.add(id);
      ...
      ).replace(/#(import.meta.ROLLUP_[^\\]+)#/g, '"+$1+"');
